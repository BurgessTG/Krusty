...
/// Chat template arguments for GLM/Kimi thinking models
...
/// Check if a model uses chat_template_args (GLM, Kimi K2)
pub fn uses_chat_template_args(model_id: &str) -> bool {
    let id_lower = model_id.to_lowercase();
    id_lower.contains("glm") || id_lower.contains("kimi-k2")
}
...
pub fn get_default_temperature(model_id: &str) -> Option<f32> {
    ...
    // Kimi K2 thinking: 1.0, regular: 0.6
    if id_lower.contains("kimi-k2") {
        return if id_lower.contains("thinking") {
            Some(1.0)
        } else {
            Some(0.6)
        };
...
///
/// Returns `Some(value)` when reasoning is ON for GLM/Kimi models:
/// - GLM models: `{ "enableThinking": true }`
/// - Kimi K2: `{ "enableThinking": true }`
///
...
pub fn get_chat_template_args(
    ...
    let is_chat_template_model = id_lower.contains("glm") || id_lower.contains("kimi-k2");
...
/// - GLM/Kimi: Handled via chat_template_args (separate field)
...
mod tests {
    ...
    fn test_is_openai_compatible_model() {
        ...
        assert!(is_openai_compatible_model("kimi-k2-thinking"));
    ...
    fn test_uses_reasoning_content() {
        ...
        assert!(!uses_reasoning_content("kimi-k2"));
    }

    #[test]
    fn test_uses_chat_template_args() {
        assert!(uses_chat_template_args("glm-4.6"));
        assert!(uses_chat_template_args("kimi-k2-thinking"));
    ...
    fn test_get_default_temperature() {
        ...
        assert_eq!(get_default_temperature("kimi-k2-thinking"), Some(1.0));
        assert_eq!(get_default_temperature("kimi-k2-turbo"), Some(0.6));
    ...
    fn test_get_chat_template_args() {
        ...
        // Kimi K2 thinking with reasoning ON
        let args = get_chat_template_args("kimi-k2-thinking", ReasoningMode::On);
        assert!(args.is_some());
        let binding = args.unwrap();
        let obj = binding.as_object().unwrap();
        assert_eq!(obj.get("enableThinking").unwrap().as_bool(), Some(true));
